...
class Workflow_List_Table extends WP_List_Table {
    ...
    function _create_columns(){        
        parent::__construct(array(
            'singular' => __('item in workflow', 'wp-better-workflow'),
            'plural' => __('items in workflow', 'wp-better-workflow'),
            'ajax' => false
        ));
        
        $this->_columns = array(
            //new WP_List_Table_Column('cb', '<input type="checkbox" />'),
            new WP_List_Table_Column('title', __('Title', 'wp-better-workflow'), true),
            new WP_List_Table_Column('post_type', __('Post Type', 'wp-better-workflow'), true),
            new WP_List_Table_Column('site', __('Site', 'wp-better-workflow'), true, true),
            new WP_List_Table_Column('last_modified', __('Date Modified', 'wp-better-workflow'), true),
        );
    }
    ...
    function column_post_type($item){
        ...
        $actions = array(
            'view_all' => '<a href="'. $item['post_type_link'] .'">' . __('View All', 'wp-better-workflow') . '</a>',
        );
    ...
    function column_site($item){
        ...
        $actions = array(
            'visit_site' => '<a href="'. $item['site_link'] .'">' . __('Visit Site', 'wp-better-workflow') . '</a>',
            'visit_dashboard' => '<a href="'. $item['site_dashboard_link'] .'">' . __('Dashboard', 'wp-better-workflow') . '</a>'
        );
        $text .= $this->row_actions($actions);
    ...
    function column_title($item){
        ...
        $actions = array(
            'view_changes' => '<a href="'. $item['compare_link'] .'">' . __('Compare Changes', 'wp-better-workflow') . '</a>',
            'edit' => '<a href="'. $item['edit_link'] .'">' . __('Edit', 'wp-better-workflow') . ' ' . $item['post_type'] . '</a>'
        );
        $text .= $this->row_actions($actions);
    ...
    /*function get_bulk_actions(){
        $actions = array(
            'approve' => __('Approve', 'wp-better-workflow'),
        );
    ...
    function get_columns(){
        global $wp_better_workflow;
        
        $columns = array();
        foreach($this->_columns as $column){
            // If not in multisite, don't include multisite columns
            if($column->multisite_only && $wp_better_workflow->is_network == false){
                continue; 
            }
    ...
    function no_items(){
        _e('No items in the workflow.', 'wp-better-workflow');
    }
    
    function prepare_items(){
        global $wpdb, $blog_id, $wp_better_workflow;
        ...
        if($wp_better_workflow->is_network){
	        ...
	        $query = apply_filters('wp_better_workflow_sites_edit_query', $query);
