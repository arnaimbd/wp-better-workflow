...
Plugin URI: http://wordpress.org/plugins/wp-better-workflow
Description: google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps google maps
Author: jamesawesome
Version: 1.0
...
define('AW_PLUGIN_DIR', WP_PLUGIN_DIR . '/wp-better-workflow');
define('AW_PLUGIN_URL', plugins_url($path = '/wp-better-workflow'));

require_once(ABSPATH . 'wp-admin/includes/plugin.php');    // Needed for is_plugin_active_for_network()
require_once('options.class.php');

class wp_better_workflow {
    var $is_network;
    var $options;
    private $done;
    private $old_post_content;
    
    function wp_better_workflow(){
        $this->is_network = (function_exists('is_plugin_active_for_network') ? is_plugin_active_for_network('wp-better-workflow/wp-better-workflow.php') : false);
        $this->options = new wp_better_workflow_Options('wp-better-workflow', $this->is_network);
    ...
    function admin_menu(){
        if(!$this->is_network){
            add_options_page(_('WP Better Workflow Options'), _('WP Better Workflow'), 'manage_options', 'wp-better-workflow-options', array(&$this, 'plugin_options'));
        }
        add_menu_page(_('Workflow'), _('Workflow'), $this->options->approval_role, 'wp-better-workflow', array(&$this, 'view_workflow'), AW_PLUGIN_URL . '/images/arrow_join.png');
    }
    
    function admin_menu_network(){
        add_submenu_page('settings.php', _('WP Better Workflow Options'), _('WP Better Workflow'), 'manage_network_options', 'wp-better-workflow-options', array(&$this, 'plugin_options'));
	    add_menu_page(_('Workflow'), _('Workflow'), 'manage_sites', 'wp-better-workflow', array(&$this, 'view_workflow'), AW_PLUGIN_URL . '/images/arrow_join.png');
    }
    ...
    function admin_notices(){
        ...
        if($pagenow == 'post.php'){
            ...
            $waiting_for_approval = __('This item is currently in the workflow. The content on this page might not reflect what is on the actual website.', 'wp-better-workflow');
            $in_progress = __('This item is currently in progress. The content on this page might not reflect what is on the actual website.', 'wp-better-workflow');
            
            if(isset($custom['_waiting_for_approval']) && $custom['_waiting_for_approval'][0]){
?>
				<div class='error'>
					<p><?php echo apply_filters('wp_better_workflow_message_waiting_for_approval', $waiting_for_approval, $post); ?></p>
				</div>
<?php
            } else {
...
?>
					<div class='updated'>
						<p><?php echo apply_filters('wp_better_workflow_message_in_progress', $in_progress, $post); ?></p>
					</div>
<?php
                }
            }
    ...
    function notify_subject($post){
        ...
        return apply_filters('wp_better_workflow_notify_subject', $subject, $post);
    }
    ...
    function notify_body($post){
        ...
        return apply_filters('wp_better_workflow_notify_body', $body, $post);
    }
    
    function plugin_options(){
?>
	<div class="wrap">
	<h2><?php _e('WP Better Workflow Options')?></h2>
    ...
    ?>
    		<script>
				window.location="<?php echo $current_page ?>?page=wp-better-workflow-options&updated=true&updatedmsg=<?php echo urlencode(__('Settings Saved')); ?>";
			</script>
    <?php
		    break;
        ...
        default:
    ?>
    		<form method="post" action="<?php echo $current_page ?>?page=wp-better-workflow-options&action=update">
    		...
    		<?php settings_fields('wp-better-workflow_group'); ?>
...
?>
	<div class="wrap">
		...
		<form method="post">
			<input type="hidden" name="page" value="wp-better-workflow" />
...
$wp_better_workflow = new wp_better_workflow();
?>